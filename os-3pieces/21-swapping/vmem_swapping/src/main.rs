...
impl MemoryManager {
    fn new(max_physical_pages: usize) -> Self {
        MemoryManager {
            ...
            allocated_pages: VecDeque::new(),
            max_physical_pages,
        }
    }

    fn allocate_page(&mut self) -> u64 {
        if self.allocated_pages.len() >= self.max_physical_pages {
            let victim_page = self.allocated_pages.pop_front().unwrap();
            self.swap_out(victim_page);
        }
        let addr = self.next_free_page;
        self.next_free_page += 0x1000;
        self.allocated_pages.push_back(addr);
        addr
    }

    fn swap_out(&mut self, pfn: u64) {
        println!("Swapping out page: 0x{:x}", pfn);
        self.swap_space.insert(pfn, pfn); // Simulating swap file
        self.physical_memory.remove(&pfn);
    }
    ...
    fn swap_in(&mut self, vpn: u64) -> u64 {
        if let Some(swapped_page) = self.swap_space.remove(&vpn) {
            ...
            self.allocated_pages.push_back(swapped_page);
            return swapped_page;
        }
        self.allocate_page()
    }
